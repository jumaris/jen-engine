...
type
  ...
  TVertexBuffer = (Buffer2, Buffer3, Buffer4{, Buffer5, Buffer6{, Buffer12, Buffer13, Buffer14, Buffer15, Buffer16, Buffer22, Buffer23, Buffer24, Buffer25, Buffer26,Buffer32, Buffer33, Buffer34, Buffer35, Buffer36,Buffer42, Buffer43, Buffer44, Buffer45, Buffer56});
  ...
  private
    ...
    FBatch        : Boolean;
    FCurrentVB    : TVertexBuffer;
    FNormalShader : IShaderResource;
    FTextShader   : IShaderResource;
    FIdx          : LongWord;
    FVrtBuff      : array[TVertexBuffer] of IGeomBuffer;
    RenderEntity  : array[TVertexBuffer] of IRenderEntity;
...
var
  ...
  VertexBuff : array[0..Batch_Size * 4 - 1] of TVertex;
...
begin
  ...
  IdxBuff := TGeomBuffer.Create(gbIndex, (Batch_Size * 6) - 2, 1, @IIdxBuff[0]);

  for Buffer := Low(TVertexBuffer) to High(TVertexBuffer) do
  begin
    FVrtBuff[Buffer] := TGeomBuffer.Create(gbVertex, Batch_Size * 4, SizeOf(VertexBuff[0]), nil);
    RenderEntity[Buffer] := Render.CreateRenderEntity;
    RenderEntity[Buffer].AttachAndBind(IdxBuff);
    RenderEntity[Buffer].AttachAndBind(FVrtBuff[Buffer]);
    RenderEntity[Buffer].BindAttrib(0, atVec4f, SizeOf(VertexBuff[0]), 0);
//    RenderEntity[Buffer].BindAttrib(1, atVec4f, SizeOf(VertexBuff[0]), 8);
  end;
...
begin
  ...
  begin
    ...
    if Assigned(DBUniform) then
      ...
      FVrtBuff[FCurrentVB].SetData(0, FIdx * 4 * 32, @FVertexBuff[1]);

    if FCurrentVB = High(TVertexBuffer) then
    FCurrentVB := Low(TVertexBuffer)
  else
    Inc(FCurrentVB);

    RenderEntity[FCurrentVB].Draw(gmTriangleStrip, FIdx * 6 - 2);
